"""
Main training worker that processes training jobs with support for SDXL and FLUX models.
"""
...
from training_worker.config_presets.config_presets import (
...
class TrainingWorker:
    """Main training worker class that processes training jobs."""
    ...
    def create_training_config(self, job_config: TrainingJobConfig) -> Dict[str, Any]:
        """
        Create the full training configuration from job parameters.
        ...
        training_config = get_config_for_training_type(
            job_config.training_type,
            **config_params
        )
        
        # Set job-specific metadata
        training_config["job_id"] = job_config.job_id
        training_config["seed"] = hash(job_config.job_id) % 2**31  # Reproducible seed from job ID
        
        return training_config
    
    def process_sdxl_job(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """Process SDXL training job (LoRA, Textual Inversion, or Finetune)."""
        try:
            training_type = config["type"]
            logger.info(f"Starting {training_type} training job")
            
            if training_type == "SDXL_LORA":
                from invoke_training.pipelines.stable_diffusion_xl.lora.config import SdxlLoraConfig
                from invoke_training.pipelines.stable_diffusion_xl.lora.train import train
                
                # Create config object and run training
                training_config = SdxlLoraConfig(**config)
                train(training_config)
                
            elif training_type == "SDXL_TEXTUAL_INVERSION":
                from invoke_training.pipelines.stable_diffusion_xl.textual_inversion.config import SdxlTextualInversionConfig
                from invoke_training.pipelines.stable_diffusion_xl.textual_inversion.train import train
                
                training_config = SdxlTextualInversionConfig(**config)
                train(training_config)
                
            elif training_type == "SDXL_FINETUNE":
                from invoke_training.pipelines.stable_diffusion_xl.finetune.config import SdxlFinetuneConfig
                from invoke_training.pipelines.stable_diffusion_xl.finetune.train import train
                
                training_config = SdxlFinetuneConfig(**config)
                train(training_config)
            
            logger.info(f"Completed {training_type} training job successfully")
            return {"status": "completed", "type": training_type}
            
        except Exception as e:
            logger.error(f"Error in SDXL training: {e}")
            logger.error(traceback.format_exc())
            raise
    
    def process_flux_job(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """Process FLUX LoRA training job."""
        try:
            logger.info("Starting FLUX LoRA training job")
            logger.info(f"GPU Memory: {self.gpu_memory_gb:.1f}GB")
            logger.info(f"Batch size: {config.get('train_batch_size', 1)}")
            logger.info(f"Gradient accumulation: {config.get('gradient_accumulation_steps', 4)}")
            logger.info(f"Resolution: {config.get('data_loader', {}).get('resolution', 768)}")
            ...
            training_config = FluxLoraConfig(**config)
            
            # Log memory optimizations being used
            logger.info(f"Memory optimizations enabled:")
            logger.info(f"  - Cache VAE outputs: {training_config.cache_vae_outputs}")
            logger.info(f"  - Cache text encoder outputs: {training_config.cache_text_encoder_outputs}")
            logger.info(f"  - Gradient checkpointing: {training_config.gradient_checkpointing}")
            logger.info(f"  - Weight dtype: {training_config.weight_dtype}")
            
            # Run FLUX training
            train(training_config)
            
            logger.info("Completed FLUX LoRA training job successfully")
            return {"status": "completed", "type": "FLUX_LORA"}
            
        except Exception as e:
            logger.error(f"Error in FLUX training: {e}")
            logger.error(traceback.format_exc())
            raise
    
    def process_job(self, job_config: TrainingJobConfig) -> Dict[str, Any]:
        """
        Process a training job end-to-end.
        ...
        try:
            ...
            if training_type == "FLUX_LORA":
                result = self.process_flux_job(training_config)
            elif training_type in ["SDXL_LORA", "SDXL_TEXTUAL_INVERSION", "SDXL_FINETUNE"]:
                result = self.process_sdxl_job(training_config)
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="AI Training Worker with FLUX and SDXL support")
